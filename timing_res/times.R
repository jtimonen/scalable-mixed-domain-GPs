...
# Plot
library(ggplot2)
plt <- ggplot(df, aes(x=n, y=time, color=node)) + geom_point(pch=4) + 
  stat_smooth(method = "lm", col = "gray30") + theme_minimal() +
  ylab("Runtime per chain (s)") + xlab("n")  +
  ggtitle("Model: y ~ age + age | z", subtitle= "J=2, R=0+2=2, M=25, L=3.0") +
  ylim(0, 2500) + xlim(0, 5000)