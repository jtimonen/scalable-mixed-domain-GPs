...
#include data.stan
  ...
  int<lower=1> C2_rank;
  int<lower=1> C3_rank;
  vector[C2_size] C2_vecs[C2_rank];
  vector[C3_size] C3_vecs[C3_rank];
  real C2_vals[C2_rank];
  real C3_vals[C3_rank];
...
transformed data{
  ...
  matrix[num_obs, num_bf*C2_rank] PSI_2;
  matrix[num_obs, 1*C3_rank] PSI_3;
...
transformed parameters {
  vector[num_obs] f_latent[num_comps];
  {
    ...
    vector[num_bf*C2_rank] d2 = STAN_diag_spd_eq(alpha[2], ell[2], seq_C2, L);
    vector[C3_rank] d3 = rep_vector(alpha[3], C3_rank); //STAN_diag_spd_eq(alpha[3], ell[3], seq_C3, L);
